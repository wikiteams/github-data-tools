...
class EventsGetter():
    ...
    def print_to_csv(self, data, repI):
        with open('csv/export_%s.csv' % repI, 'a+') as f:
            f.write("repository_owner;repository_url;repository_name;issue;status;opened_at;closed_at;diff_in_minutes;diff_in_hours;diff_in_days\r\n")
            ...
            for item in data:
                ...
                f.write('"%(repository_owner)s";"%(repository_url)s";"%(repository_name)s";"%(issue)s";"%(status)s";"%(opened_at)s";"%(closed_at)s";"%(diff_in_minutes)s";"%(diff_in_hours)s";"%(diff_in_days)s"\r\n' % item)
        ...
        for repository in repositories:
            ...
            try:
                ...
                for event in events:
                    ...
                    corelatedEvents = list(self.db.wikiteams.events.find({ "payload.issue": event }).sort("created_at"))

                    tmp = {'repository_owner': repository['repository_owner'], 'repository_url': repository['repository_url'], 'repository_name': repository['repository_name'], 'issue': corelatedEvents[0]['payload']['issue']}
